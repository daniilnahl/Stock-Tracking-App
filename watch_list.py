...
class Watch_list:
    ...
    def show_stocks(self):
        ...
        for stock in self.stocks: #calls stock attributes 
           table.add_row(stock.exchange, stock.ticker_symbol, stock.name, stock.market_cap, stock.amount_owned, stock.cost_basis, stock.current_price, f"[{Watch_list.get_color(float(stock.total_return))}]{stock.total_return + "%"}[/]" , stock.price_1d + "%" , stock.price_5d + "%", 
